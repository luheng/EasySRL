...
public class QueryOld {
     ...
     */
    public QueryOld(List<String> question, int predicateIndex, int numTotalParses) {
    ...
    public QueryOld(int predicateIndex, int numTotalParses, List<String> question, double questionScore,
    ...
    public void print(List<String> sentence, Response response) {
        ...
        System.out.println(String.format("%.3f\t%s?", entropy, getQuestionString()));
        answerToParses.keySet().stream().sorted().forEach(id -> {
            String answerStr = (id < 0) ? "N/A" : sentence.get(id);
