...
public abstract class BilexicalFeature extends Feature {
	...
	public FeatureKey getFeatureKey(final List<InputWord> words, final SRLLabel role, final int predicateIndex,
									final int argumentIndex) {
		return getFeatureKey2(words, role, predicateIndex, argumentIndex);
	}

	abstract FeatureKey getFeatureKey2(List<InputWord> words, SRLLabel role, int predicateIndex, int argumentIndex);
	 ...
	 */
	public static Collection<BilexicalFeature> getBilexicalFeatures(final List<Clustering> clusterings,
																	final int window) {
		...
		for (final Clustering clustering : clusterings) {
			dependencyFeaturesWithClustering.add(new LemmaAndRoleAndArgCluster(new LexicalFeature(clustering, 0)));
		}
		dependencyFeaturesWithClustering.add(new LemmaAndRoleAndArgCluster(new POSfeature(0)));
		for (int i = -window; i <= window; i++) {
			for (final Clustering clustering : clusterings) {
				dependencyFeaturesWithClustering.add(new ContextDependencyFeature(true, new LexicalFeature(clustering, i)));
				dependencyFeaturesWithClustering.add(new ContextDependencyFeature(false, new LexicalFeature(clustering, i)));
			}
			dependencyFeaturesWithClustering.add(new ContextDependencyFeature(true, new POSfeature(i)));
			dependencyFeaturesWithClustering.add(new ContextDependencyFeature(false, new POSfeature(i)));
		}
		dependencyFeaturesWithClustering.add(new DistanceDependencyFeature(10, null));
		dependencyFeaturesWithClustering.add(new LemmaAndRoleAndArgCapitalization());
	...
	private static class LemmaAndRoleAndArgCluster extends BilexicalFeature {
		...
		FeatureKey getFeatureKey2(final List<InputWord> words, final SRLLabel role, final int predicateIndex,
			...
			return hash(super.id, MorphaStemmer.stemToken(words.get(predicateIndex).word).hashCode(), role.hashCode(),
					argumentFeature.getValue(words, argumentIndex).hashCode());
		}
	}
	...
	private static class ContextDependencyFeature extends BilexicalFeature {
		...
		private ContextDependencyFeature(final boolean predicateContext, final LexicalFeature feature) {
			super();
		...
		FeatureKey getFeatureKey2(final List<InputWord> words, final SRLLabel role, final int predicateIndex,
			...
			final FeatureKey key = hash(super.id, role.hashCode(), feature.getOffset(), value.hashCode());
			return key;
		}
	...
	private static class DistanceDependencyFeature extends BilexicalFeature {
		...
		private DistanceDependencyFeature(final int maxValue, final Integer includeWordAtOffset) {
			super();
		...
		FeatureKey getFeatureKey2(final List<InputWord> words, final SRLLabel role, final int predicateIndex,
			...
			if (includeWordAtOffset == null) {
				...
				return hash(super.id, role.hashCode(), normalizedDistance, getWord(words, includeWordAtOffset).hashCode());
			}
		}
